...
const Input: React.FC<InputProps> = memo(({ name, ...rest }) => {
	...
	return <input name={name} {...rest} ref={inputRef} />;
});

const InputRadio: React.FC<InputRadioProps> = memo(
	({ isFilterOpened, name, btnType = 'button', options, ...rest }) => {
		...
		const setRefs = useCallback(
			ref => {
				if (ref !== null) {
					const foundIndexRef = inputsRefs.length
						? inputsRefs.findIndex(item => item.id === ref.id)
						: -1;
